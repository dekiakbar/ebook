	...
	if (isset($_POST['apdet'])) {
				...
				if (mysqli_num_rows($tes) == 0) {
					if (mysqli_num_rows($apass) == 1) {
							$insert = mysqli_query($koneksi,"UPDATE buku SET jdl='$judul', nmp='$pengarang', kat='$kat', link=link, tgl='$tgl', donasi='$donasi', ukuran=ukuran WHERE id='$id'") or die('Error: ' . mysqli_error($koneksi));
							if ($insert) {
								echo "<script>swal('Berhasil','Data Berhasil Diperbaharui','success');</script>";
							}else{
								echo "<script>swal('Gagal','Data Gagal Diperbaharui','error');</script>";
							}
						}else{
